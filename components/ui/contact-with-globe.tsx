...
import { ArrowRight, Mail, MapPin, Building2, CheckCircle2, Loader2, AlertCircle } from "lucide-react";
...
const CONTACT_LINKS = [
  ...
  {
    icon: MapPin,
    label: "Unit3F-03, Level 3, CENTURY CENTRAL, Mango Garden Layout, Bikasipura, Bengaluru 560062",
    href: "#",
  },
];
...
export default function ContactWithGlobe({
  title = "Contact us",
  subtitle = "Contact",
  description = "We are always looking for ways to partner with ambitious brands. Reach out and let us know how we can help you.",
  className,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const templateId = process.env.NEXT_PUBLIC_EMAILJS_TEMPLATE_ID || "template_ojoe2ck";
    const autoReplyTemplateId = process.env.NEXT_PUBLIC_EMAILJS_AUTO_REPLY_TEMPLATE_ID || "template_cqkxh1r";
    ...
    if (
      ...
      console.warn("EmailJS Service ID and Public Key need to be filled in .env.local. Simulating success.");
    ...
    try {
      if (formRef.current) {
        // Send main contact notification (Contact Us: template_ojoe2ck)
        const sendContact = emailjs.sendForm(serviceId, templateId, formRef.current, publicKey);
        
        // Send automatic reply to visitor (Auto-Reply: template_cqkxh1r)
        const sendAutoReply = emailjs
          .sendForm(serviceId, autoReplyTemplateId, formRef.current, publicKey)
          .catch((err) => console.warn("Auto-reply send notice:", err));

        await Promise.all([sendContact, sendAutoReply]);
      }
      setSubmitted(true);
    } catch (err: any) {
      console.error("EmailJS submission error:", err);
      setErrorMessage("Failed to send message via EmailJS. Please email us directly at info@blyntech.com");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <section
      className={cn(
        "relative w-full bg-ink text-white overflow-hidden py-20 min-h-screen pt-28",
        className,
      )}
    >
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6">
        <div className="flex flex-col items-center text-center gap-4 mb-12">
          <motion.div
            ...
            className="inline-flex items-center px-4 py-1.5 rounded-full bg-white/10 border border-white/20"
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 max-w-5xl mx-auto items-start">
          ...
          >
            <div className="flex flex-col gap-1">
              ...
              <p className="text-sm text-white/60 leading-relaxed max-w-sm">
                Reach out via email or visit our headquarters below. We typically reply within one business day.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-4">
              {CONTACT_LINKS.map(({ icon: Icon, label, href }, i) => (
                <motion.a
                  ...
                  className="group flex items-start gap-3 w-fit text-sm text-white/70 hover:text-white transition-colors duration-200"
                >
                  <div className="w-8 h-8 rounded-lg bg-white/10 border border-white/20 group-hover:border-white/50 flex items-center justify-center shrink-0 transition-all duration-200 mt-0.5">
            ...
            <div className="relative overflow-hidden h-72 rounded-2xl border border-white/10 bg-white/5 p-4 mt-2">
              <GlobeWireframe
          ...
          <motion.div
            ...
            className="rounded-3xl border border-white/15 bg-white/5 backdrop-blur-md p-6 sm:p-8 flex flex-col gap-5 shadow-2xl"
          >
            {submitted ? (
              <div className="py-12 text-center">
                <CheckCircle2 size={56} className="text-accent mx-auto mb-4 animate-bounce" />
                <h3 className="font-display text-3xl font-bold text-white">Message sent!</h3>
                <p className="mt-4 text-white/70 text-sm max-w-xs mx-auto">
                  Thank you for reaching out. We will get back to you at your email address promptly from info@blyntech.com.
                </p>
              </div>
            ) : (
              <form ref={formRef} onSubmit={handleSubmit} className="flex flex-col gap-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="flex flex-col gap-2">
                    <label className="text-xs font-semibold tracking-widest uppercase text-white/50 font-mono">
                      Full Name
                    </label>
                    <input
                      type="text"
                      ...
                      className="w-full bg-ink/80 border border-white/20 rounded-xl px-4 py-2.5 text-sm text-white placeholder:text-white/30 outline-none focus:border-white focus:ring-2 focus:ring-white/10 transition-all duration-200"
                    />
                  </div>
                  <div className="flex flex-col gap-2">
                    <label className="text-xs font-semibold tracking-widest uppercase text-white/50 font-mono">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <textarea
                    ...
                    rows={4}
                    className="w-full bg-ink/80 border border-white/20 rounded-xl px-4 py-3 text-sm text-white placeholder:text-white/30 outline-none focus:border-white focus:ring-2 focus:ring-white/10 resize-none transition-all duration-200"
                  />
                </div>
                ...
                <Button
                  type="submit"
                  disabled={loading}
                  className="w-fit h-11 px-8 rounded-xl font-semibold text-sm bg-white text-ink hover:bg-white/90 group cursor-pointer disabled:opacity-50"
                >
                  {loading ? (
                    <>
